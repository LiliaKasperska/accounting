        ...
        <div class="row" style="height: 100%;">
            ...
            <div class="col-sm-2 " style=" background-color: #b1c840;">
                <div class="btn-group-vertical nav justify-content-end" role="group" aria-label="Basic example">
                    <a href="drivers_list.php"><button type="button" class="btn2 btn-dark2">Список водіїв</button></a>
                    <div style="height: 15px;"></div>
                    <a href="driver_new.php?a=0"><button type="button" class="btn2 btn-dark2" <?php if ($_SESSION['id_d']=='0'){ ?> autofocus <?php } ?>>Додати водія</button></a>
                </div>
            </div>
            <div class="col-sm-3">
            <div style="height: 5%;"></div> 
                <?php
                    if($_SESSION['id_d']!='0'){
                        $sql="SELECT * FROM drivers WHERE id='".$_SESSION['id_d']."'";
                        $res=mysqli_query($connect,$sql);
                        $result=mysqli_fetch_array($res);
                    }
                ?>
                <form method="POST" action="driver_new.php">
                    Прізвище, ім'я: <input type="text" class="form-control" name="name" <?php if($_SESSION['id_d']!='0'){ ?> value="<?php echo $result['name']; ?>" <?php } ?>><br>
                    Логін: <input type="text" class="form-control" name="login" <?php if($_SESSION['id_d']!='0'){ ?> value="<?php echo $result['login']; ?>" <?php } ?>><br>
                    Телефон: <input type="text" class="form-control" name="phone" <?php if($_SESSION['id_d']!='0'){ ?> value="<?php echo $result['phone']; ?>" <?php } ?>><br>
                    <?php if($_SESSION['id_d']!='0'){?>
                ...
                <?php
                    if(isset($_POST['save_new'])){
                        if($_POST['pass_f']==$_POST['pass_s'] and !empty($_POST['pass_f']) and !empty($_POST['login']) and !empty($_POST['name']) and !empty($_POST['phone'])){
                            $sql="INSERT INTO drivers (name, login, password, phone) VALUES ('".$_POST['name']."','".$_POST['login']."','".$_POST['pass_f']."','".$_POST['phone']."')";
                            $res=mysqli_query($connect,$sql);
                            ?>
                    ...
                    if(isset($_POST['save'])){
                        if($_POST['pass_f']==$_POST['pass_s'] and !empty($_POST['pass_f'])){
                            $sql="UPDATE drivers SET password='".$_POST['pass_f']."' WHERE id='".$_SESSION['id_d']."'";
                            $res=mysqli_query($connect,$sql);
                            ?>
                            <script>document.location.href="drivers_list.php"</script>
                            <?php
                    ...
                    if(isset($_POST['save1'])){
                        if(!empty($_POST['login']) and !empty($_POST['name']) and !empty($_POST['phone'])){
                            $sql="UPDATE drivers SET name='".$_POST['name']."', login='".$_POST['login']."', phone='".$_POST['phone']."' WHERE id='".$_SESSION['id_d']."'";
                            $res=mysqli_query($connect,$sql);
                            ?>
                            <script>document.location.href="drivers_list.php"</script>
                            <?php
