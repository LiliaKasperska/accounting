        ...
        <div class="row" style="height: 100%;">
            ...
            <div class="col-sm-2 " style=" background-color: #b1c840;">
                <div class="btn-group-vertical nav justify-content-end" role="group" aria-label="Basic example">
                    ...
                    <div style="height: 15px;"></div>
                    <a href="setings_destination.php"><button type="button" class="btn2 btn-dark2" autofocus>Пункти призначення</button></a>
                </div>
            </div>
            <div class="col-sm-6">
            <div style="height: 5%;"></div> 
                <?php
                    ...
                    while($result=mysqli_fetch_array($res)){
                        ?>
                        <form method="POST" action="setings_destination.php">
                        ...
                        if(isset($_POST[$result['id']])){
                            if(!empty($_POST['text'])){
                                $sql="UPDATE destination SET name='".$_POST['text']."' WHERE id='".$result['id']."'";
                                $res=mysqli_query($connect,$sql);
                                ?>
                                <script>document.location.href="setings_destination.php"</script>
                                <?php
                            } else {
                                $sql="DELETE FROM destination WHERE id='".$result['id']."'";
                                $res=mysqli_query($connect,$sql);
                                ?>
                                <script>document.location.href="setings_destination.php"</script>
                                <?php
                ...
                <form method="POST" action="setings_destination.php">
                    <div class="input-group mb-3 col-sm-6">
                        <input type="text" class="form-control" placeholder="Введіть новий пункт" aria-describedby="button-addon2" name="txt">
                        <div class="input-group-append">
                ...
                <?php 
                    if(isset($_POST['save'])){
                        if(!empty($_POST['txt'])){
                            $sql="INSERT INTO destination (name) VALUES ('".$_POST['txt']."')";
                            $res=mysqli_query($connect,$sql);
                            ?>
                            <script>document.location.href="setings_destination.php"</script>
                             <?php
